...
then
    ...
    then
        ...
        collectionCannotAirPostTime=$(echo "$getCollectionInfoHttpResponse" | awk -F '<f_sp_cannotAirPostTime__values__str>|</f_sp_cannotAirPostTime__values__str>' '{print $2}')
        collectionCannotAirPreTime=$(echo "$getCollectionInfoHttpResponse" | awk -F '<f_sp_cannotAirPreTime__values__str>|</f_sp_cannotAirPreTime__values__str>' '{print $2}')
        ...
        then
            ...
            if [[ "$collectionCannotAirPostTime" == *list-item* ]];
            then
                collectionCannotAirPostTimeValues=$(createCommaSeperatedList "$collectionCannotAirPostTime" "collectionContentDescriptors")
            else
                collectionCannotAirPostTimeValues=$(echo "$collectionCannotAirPostTime")
            fi
            if [[ "$collectionCannotAirPreTime" == *list-item* ]];
            then
                collectionCannotAirPreTimeValues=$(createCommaSeperatedList "$collectionCannotAirPreTime" "collectionContentDescriptors")
            else
                collectionCannotAirPreTimeValues=$(echo "$collectionCannotAirPreTime")
            fi
            ...
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (spWorkflow) - ($collectionId) - Final Collection Cannot Air Post Time - [$collectionCannotAirPostTimeValues]" >> "$logfile"
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (spWorkflow) - ($collectionId) - Final Collection Cannot Air Pre Time - [$collectionCannotAirPreTimeValues]" >> "$logfile"
...
Cannot Air Post Time - $collectionCannotAirPostTimeValues
Cannot Air Pre Time - $collectionCannotAirPreTimeValues

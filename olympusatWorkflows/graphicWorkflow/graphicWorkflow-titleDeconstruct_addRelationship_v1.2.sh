...
processImageContent() {
    export imageContentTypeCheck=$(filterVidispineItemMetadata $itemId "metadata" "oly_contentType")
    if [[ "$imageContentTypeCheck" != "image" ]];
    then
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - INTERNAL FUNCTION - Content Type is NOT image - [$imageContentTypeCheck] - skipping & exiting the Script/Workflow" >> "$logfile"
    else
        ...
        then
            ...
            then
                ...
                itemSearchItemContentType=$(echo $itemSearchHttpResponse | awk -F "<oly_contentType>" '{print $2}' | awk -F "</oly_contentType>" '{print $1}')
                itemSearchItemContentType=$(echo $itemSearchItemContentType | awk -F "<list-item>" '{print $2}' | awk -F "</list-item>" '{print $1}')
                ...
                echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Search Results - Item Content Type - [$itemSearchItemContentType]" >> "$logfile"
                ...
                sleep 2
                if [[ "$itemSearchItemContentType" == "episode" || "$itemSearchItemContentType" == "movie" ]];
                then
                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - [$itemId] - Triggering Item Validation Workflow on [$itemSearchItemId]" >> "$logfile"
                    bash -c "sudo /opt/olympusat/scriptsActive/mediaManagerWorkflow-itemValidation_v1.5.sh $itemSearchItemId > /dev/null 2>&1 &"
                    sleep 2
                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - [$itemId] - Trigger Item Validation Workflow COMPLETED" >> "$logfile"
                fi
...
then
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Gathering Information from [$itemContentType]" >> "$logfile"
    export itemVersionType=$(filterVidispineItemMetadata $itemId "metadata" "oly_versionType")
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Version Type [$itemVersionType]" >> "$logfile"
    ...
    then
        ...
        then
            export itemTitleCode=$(filterVidispineItemMetadata $itemId "metadata" "oly_titleCode")
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Title Code [$itemTitleCode]" >> "$logfile"
            export itemImageSearchUrl="http://10.1.1.34/API/v2/search/"
            itemImageSearchBody="{ \"filter\": { \"operator\": \"AND\",\"terms\": [{ \"name\": \"oly_titleCode\", \"value\": \"$itemTitleCode\", \"exact\": true },{ \"name\": \"oly_contentType\", \"value\": \"image\" },{ \"name\": \"oly_graphicsType\", \"value\": \"still\" }]}}"
            itemImageSearchHttpResponse=$(curl --location --request PUT $itemImageSearchUrl --header 'Content-Type: application/json' --header 'Accept: application/xml' --header 'Authorization: Basic YWRtaW46MTBsbXBAc0B0' --header 'Cookie: csrftoken=VDa9RP3Y9rgomyzNWvRxbu7WdTMetVYBlLg6pGMIJ6oyVABsjJiiEK9JCWVA1HYd' --data $itemImageSearchBody)
            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Search Results - HTTP Response - [$itemImageSearchHttpResponse]" >> "$logfile"
            itemImageHitResults=$(echo $itemImageSearchHttpResponse | awk -F "<hits>" '{print $2}' | awk -F "</hits>" '{print $1}')
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Search Results - Number of Hits - [$itemImageHitResults]" >> "$logfile"
            if [ "$itemImageHitResults" -eq 1 ];
            then
                imageItemId=$(echo $itemImageSearchHttpResponse | awk -F "<id>" '{print $2}' | awk -F "</id>" '{print $1}')
                echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Image Item ID - [$imageItemId]" >> "$logfile"
                sleep 2
                itemId=$(echo "$imageItemId")
                echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Triggering Jump to processImageContent" >> "$logfile"
                # Jump to Image processing
                processImageContent
            else
                echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Number of Images in Search Results NOT Supported - [$itemImageHitResults]" >> "$logfile"
            fi
        else
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Item is NOT Original Raw Master - skipping & exiting the Script/Workflow" >> "$logfile"
        fi
    ...
    then
        ...
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Version Type is NOT Supported - skipping & exiting the Script/Workflow" >> "$logfile"
    fi
elif [[ "$itemContentType" == "image" ]]
then
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (graphicWorkflow) - ($itemId) - Triggering Jump to processImageContent" >> "$logfile"
    # Jump to Image processing
    processImageContent

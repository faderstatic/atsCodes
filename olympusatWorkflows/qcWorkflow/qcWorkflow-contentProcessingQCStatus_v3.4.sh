...
lockFile="/opt/olympusat/workflowQueues/contentProcessingQcWorkflow/jobQueue.lock"
echo "$(date +%Y/%m/%d_%H:%M:%S) - (contentProcessingQC) - [$itemId] - Update QC Status Job Initiated - User ($user)" >> "$logfile"
sleep 1
# Acquire the lock by waiting if another job is running
while [ -f "$lockFile" ];
do
    #echo "$(date +%Y/%m/%d_%H:%M:%S) - (contentProcessingQC) - [$itemId] - Waiting for the previous job to finish..." >> "$logfile"
    sleep 3
done

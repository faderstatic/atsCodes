    ...
    then
        ...
        then
            ...
            then
                ...
                if [[ "$emailNotificationWorkflow" == "rtcMexicoQcPending" ]];
                then
                    # emailNotificationWorkflow varialbe is set to rtcMexicoQcPending
                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - ($itemId) - Checking for rtcMexicoQcPendingFileDestination file" >> "$logfile"

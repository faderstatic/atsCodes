...
MAM Notify"
                ...
                if [[ "$notificationType" == "rtcMexicoQcPending" ]];
                then
                    # notificationType is 'rtcMexicoQcPending'-continue with workflow
                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - RTC Mexico QC Pending Workflow Triggered - Check for List to Send Email" >> "$logfile"

                    rtcMexicoQcPendingFileDestination="/opt/olympusat/resources/emailNotificationWorkflow/rtcMexicoQcPending/rtcMexicoQcPending-$mydate.csv"

                    # Check rtcMexicoQcPendingFileDestination Variable
                    if [[ -e "$rtcMexicoQcPendingFileDestination" ]];
                    then
                        # rtcMexicoQcPendingFileDestination file exists - continuing with script/workflow
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - rtcMexicoQcPendingFileDestination file exists - continuing with script/workflow" >> "$logfile"
...
MAM Notify"
                        ...
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - To - $recipient1, $recipient2, $recipient3, $recipient4, $recipient5" >> "$logfile"
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - From - $emailFrom" >> "$logfile"
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - Subject - $subject" >> "$logfile"
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - Body - [$(echo $body)]" >> "$logfile"

                        # Setup to send email with attachment
                        sesSubject=$(echo $subject) 
                        sesMessage=$body
                        sesFile=$(echo $rtcMexicoQcPendingFileDestination)
                        sesMIMEType=`file --mime-type "$sesFile" | sed 's/.*: //'`
                        ...
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-rtcMexicoQcPending) - Email Sent Successfully" >> "$logfile"

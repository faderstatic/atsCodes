...
MAM Notify"
    ...
    ***NOTE***: You must be on the Olympusat Office Network, either via VPN or remoting into a machine on the Network, in order to access Cantemo Web Portal
    ...
    MAM Notify"
                            ...
                            if [[ "$notificationType" == "adComplianceNewItem" ]];
                            then
                                # notificationType is 'adComplianceNewItem'-continue with workflow
                                echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Prepare For Reload Workflow Triggered - Check for List to Send Email" >> "$logfile"
                                ...
                                # Check adComplianceNewItemFileDestination Variable
                                if [[ -e "$adComplianceNewItemFileDestination" ]];
                                then
                                    # adComplianceNewItemFileDestination file exists - continuing with script/workflow
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - adComplianceNewItemFileDestination file exists - continuing with script/workflow" >> "$logfile"

                                    # Recipient email addresses
                                    export recipient1=dbassett@olympusat.com
                                    export recipient2=mamAdmin@olympusat.com
                                    export recipient3=rsims@olympusat.com
                                    export recipient4=kkanjanapitak@olympusat.com
                                    ...
                                    # Email Body
                                    subject="MAM - Ad Compliance - New Content - $mydate"
                                    body="Hi,
                        
The following attached list of items have been ingested into Cantemo and marked as Review Status Pending.
...
MAM Notify"
                                    ...
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Sending Email for Prepare For Reload Items" >> "$logfile"
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - To - $recipient1, $recipient2, $recipient3, $recipient4, $recipient5" >> "$logfile"
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - From - $emailFrom" >> "$logfile"
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Subject - $subject" >> "$logfile"
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Body - [$(echo $body)]" >> "$logfile"

                                    # Setup to send email with attachment
                                    sesSubject=$(echo $subject) 
                                    sesMessage=$body
                                    sesFile=$(echo $adComplianceNewItemFileDestination)
                                    sesMIMEType=`file --mime-type "$sesFile" | sed 's/.*: //'`
                                    ...
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Email Sent Successfully" >> "$logfile"

                                    sleep 2

                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Moving adComplianceNewItem csv to zCompleted folder" >> "$logfile"

                                    mv "$adComplianceNewItemFileDestination" "/opt/olympusat/resources/emailNotificationWorkflow/zCompleted/"

                                    sleep 2

                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-adComplianceNewItem) - Prepare For Reload Email Notification Process Completed" >> "$logfile"

                                else
                                    # adComplianceNewItemFileDestination file DOES NOT exist-exiting script/workflow
                                    echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow) - adComplianceNewItemFileDestination file DOES NOT exist - exiting script/workflow" >> "$logfile"
                                fi
                            else

...
# Check Variable
if [[ "$emailNotificationWorkflow" == "newContentMissingMetadata" ]];
then
    ...
    then
        ...
        echo "ItemId,Title,Licensor,ContentType,VersionType,FileExtension,ContentFlags,OriginalQCStatus,FinalQCStatus" >> "$newContentMissingMetadataFileDestination"

        echo "$(date +%Y/%m/%d_%H:%M:%S) - (notificationWorkflow-newContentMissingMetadata) - New File created - [$newContentMissingMetadataFileDestination]" >> "$logfile"
        
        sleep 5
    fi 

    # API Call to Search for items in the 'New Content Missing Metadata' Saved Search
    searchResponse=$(curl --location --request PUT 'http://10.1.1.34/API/v2/search/?page_size=1000' \

...
echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - cantemoItemId - {$cantemoItemId}" >> "$logfile"
echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - userName - {$userName}" >> "$logfile"
echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - columnHeader - {$columnHeader}" >> "$logfile"
echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - inputFolder - {$inputFolder}" >> "$logfile"
echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - cantemoItemTitle - {$cantemoItemTitle}" >> "$logfile"
...
then
    ...
    do
        ...
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - matchedRow - {$matchedRow}" >> "$logfile"
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - inputFile - {$inputFile}" >> "$logfile"
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - cantemoItemId - {$cantemoItemId}" >> "$logfile"
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - itemIdColumn - {$itemIdColumn}" >> "$logfile"
        matchedValue=$(echo $matchedRow | awk -F ':' '{print $2}')
        echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - matchedValue - {$matchValue}" >> "$logfile"
    ...
    # --------------------------------------------------
    # Read Specific Line
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - matchedRowNumber - {$matchedRowNumber}" >> "$logfile"
    ...
    then
        ...
        do
            ...
            then
                ...
                echo "$(date +%Y/%m/%d_%H:%M:%S) - (initialIngestMetadata) - [$cantemoItemId] - Check updateInCantemo column value - [$updateInCantemoValue]" >> "$logfile"
            fi
            # --------------------------------------------------
        done

...
<MetadataDocument xmlns=\"http://xml.vidispine.com/schema/vidispine\">
  <group>Olympusat</group>
    ...
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (importContractMetadata) - [$cantemoItemId] - XML has been created {$cantemoItemId-CONTRACT.xml}" >> "$logfile"
    # --------------------------------------------------
    ...
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (importContractMetadata) - [$cantemoItemId] - Triggering API Call to Import XML into Cantemo" >> "$logfile"
    ...
    echo "$(date +%Y/%m/%d_%H:%M:%S) - (importContractMetadata) - [$cantemoItemId] - Moving XML to zCompleted Folder" >> "$logfile"

    sleep 2

    mv "$fileDestination" "/opt/olympusat/xmlsForMetadataImport/zCompleted/"

    echo "$(date +%Y/%m/%d_%H:%M:%S) - (importContractMetadata) - [$cantemoItemId] - Import Rightsline Legacy Information Completed" >> "$logfile"

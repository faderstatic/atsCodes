...
then
    ...
    then
        ...
        itemOriginalFilename=$(echo "$itemOriginalFilename" | sed 's/.mov/.mp4/g')
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Current Target Id Value [$currentTargetIdValue]" >> "$logfile"
                        stillSourceFilePath=$(filterVidispineFileInfo $currentTargetIdValue "uri" "tag=original")
                        stillSourceFilePath=$(echo "$stillSourceFilePath" | sed 's/%20/ /g')
                        #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Episodic Still Source File Path [$stillSourceFilePath]" >> "$logfile"
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Episodic Still to creative Amazon Staging Folder IN PROGRESS" >> "$logfile"
                        cp "$stillSourceFilePath" "/Volumes/creative/CS13/Distribution/Amazon_Staging/graphics/$itemTitle-episodic-16x9.jpg"
                        sleep 2
                        echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Episodic Still to creative Amazon Staging Folder COMPLETED" >> "$logfile"
                    fi
                fi
            fi
        done
            ...
            do
                ...
                then
                    ...
                    then
                        if [[ "$relatedItemType" == "cover" && "$aspectRatio" == "16x9" ]];
                        then
                            echo "            <md:ArtReference resolution=\"$relatedItemResolution\" purpose=\"cover\">$seasonItemTitle-cover-16x9.jpg</md:ArtReference>" >> "$mecSeasonFileDestinationArt"
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Current Target Id Value [$currentTargetIdValue]" >> "$logfile"
                            coverArtSourceFilePath=$(filterVidispineFileInfo $currentTargetIdValue "uri" "tag=original")
                            coverArtSourceFilePath=$(echo "$coverArtSourceFilePath" | sed 's/%20/ /g')
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - CoverArt Source File Path [$coverArtSourceFilePath]" >> "$logfile"
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season CoverArt to creative Amazon Staging Folder IN PROGRESS" >> "$logfile"
                            cp "$coverArtSourceFilePath" "/Volumes/creative/CS13/Distribution/Amazon_Staging/graphics/$seasonItemTitle-cover-16x9.jpg"
                            sleep 2
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season CoverArt to creative Amazon Staging Folder COMPLETED" >> "$logfile"
                        elif [[ "$relatedItemType" == "cover" && "$aspectRatio" == "4x3" ]];
                        then
                            echo "            <md:ArtReference resolution=\"$relatedItemResolution\" purpose=\"boxart\">$seasonItemTitle-box-4x3.jpg</md:ArtReference>" >> "$mecSeasonFileDestinationArt"
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Current Target Id Value [$currentTargetIdValue]" >> "$logfile"
                            boxArtSourceFilePath=$(filterVidispineFileInfo $currentTargetIdValue "uri" "tag=original")
                            boxArtSourceFilePath=$(echo "$boxArtSourceFilePath" | sed 's/%20/ /g')
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - BoxArt Source File Path [$boxArtSourceFilePath]" >> "$logfile"
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season BoxArt to creative Amazon Staging Folder IN PROGRESS" >> "$logfile"
                            cp "$boxArtSourceFilePath" "/Volumes/creative/CS13/Distribution/Amazon_Staging/graphics/$seasonItemTitle-box-4x3.jpg"
                            sleep 2
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season BoxArt to creative Amazon Staging Folder COMPLETED" >> "$logfile"
                        elif [[ "$relatedItemType" == "feature" && "$aspectRatio" == "16x9" ]];
                        then
                            echo "            <md:ArtReference resolution=\"$relatedItemResolution\" purpose=\"hero\">$seasonItemTitle-hero-16x9.jpg</md:ArtReference>" >> "$mecSeasonFileDestinationArt"
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Current Target Id Value [$currentTargetIdValue]" >> "$logfile"
                            heroArtSourceFilePath=$(filterVidispineFileInfo $currentTargetIdValue "uri" "tag=original")
                            heroArtSourceFilePath=$(echo "$heroArtSourceFilePath" | sed 's/%20/ /g')
                            #echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - HeroArt Source File Path [$heroArtSourceFilePath]" >> "$logfile"
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season HeroArt to creative Amazon Staging Folder IN PROGRESS" >> "$logfile"
                            cp "$heroArtSourceFilePath" "/Volumes/creative/CS13/Distribution/Amazon_Staging/graphics/$seasonItemTitle-hero-16x9.jpg"
                            sleep 2
                            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - Copy Season HeroArt to creative Amazon Staging Folder COMPLETED" >> "$logfile"
                        fi
                    fi
                fi
            done

...
then
 ...
 xmlns:mdmec=\"http://www.movielabs.com/schema/mdmec/v2.9\">" >> "$mecFileDestination"
    ...
    then
        ...
        then
            # mecSeasonFileDestination file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestination file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestination" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationArt file exists
        if [[ -e "$mecSeasonFileDestinationArt" ]];
        then
            # mecSeasonFileDestinationArt file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationArt file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationArt" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationGenre file exists
        if [[ -e "$mecSeasonFileDestinationGenre" ]];
        then
            # mecSeasonFileDestinationGenre file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationGenre file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationGenre" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationRating file exists
        if [[ -e "$mecSeasonFileDestinationRating" ]];
        then
            # mecSeasonFileDestinationRating file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationRating file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationRating" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationActor file exists
        if [[ -e "$mecSeasonFileDestinationActor" ]];
        then
            # mecSeasonFileDestinationActor file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationActor file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationActor" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationDirector file exists
        if [[ -e "$mecSeasonFileDestinationDirector" ]];
        then
            # mecSeasonFileDestinationDirector file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationDirector file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationDirector" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Check to see if mecSeasonFileDestinationProducer file exists
        if [[ -e "$mecSeasonFileDestinationProducer" ]];
        then
            # mecSeasonFileDestinationProducer file exists-deleting file before continuing
            echo "$(date +%Y/%m/%d_%H:%M:%S) - (distributionWorkflow) - ($itemId) - mecSeasonFileDestinationProducer file exists - moving file to zMoved folder before continuing with script" >> "$logfile"
            mv -f "$mecSeasonFileDestinationProducer" "/opt/olympusat/xmlsForDistribution/zMoved/"
            sleep 1
        fi
        # Gathering metadata from Cantemo
        itemContentType=$(filterVidispineItemMetadata $itemId "metadata" "oly_contentType")

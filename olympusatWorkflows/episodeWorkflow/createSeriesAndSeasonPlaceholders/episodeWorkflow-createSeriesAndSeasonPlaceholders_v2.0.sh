...
then
    ...
    then
        ...
        then
            ...
            createRelationBody="{ \"relation\": [{ \"direction\": {\"source\": \"$itemId\", \"target\": \"$seriesItemId\", \"type\": \"U\" }, \"value\": [ { \"key\": \"type\", \"value\": \"portal_undirectional\" }, { \"key\": \"cs_type\", \"value\": \"series\" }]}]}"
            createRelationHttpResponse=$(curl --location --request PUT $createRelationUrl --header 'Content-Type: application/json' --header 'Accept: application/xml' --header 'Authorization: Basic YWRtaW46MTBsbXBAc0B0' --header 'Cookie: csrftoken=VDa9RP3Y9rgomyzNWvRxbu7WdTMetVYBlLg6pGMIJ6oyVABsjJiiEK9JCWVA1HYd' --data $createRelationBody)

...
# 	Usage: $s3ActionNameQueue.sh [script to execute - full path] [folder location of queue]
...
export queueListFile="/opt/olympusat/$s3ActionName/Queue$myDateTime-$s3ActionName.txt"
...
echo "$(date "+%H:%M:%S") ($s3ActionName""Queue) - Start processing queue" >> "$logFile"
#------------------------------
...
echo "$(date "+%H:%M:%S") ($s3ActionName""Queue) - There are now $pendingJobCount item(s) in the queue" >> "$logFile"
...
do
    ...
    do
        ...
        do
            echo "$(date "+%H:%M:%S") ($s3ActionName""Queue) - Active jobs for $s3ActionName reaches its limit ($concurrentLimit)" >> "$logFile"
            sleep 300
            queueCount=$(ls $activeFolder | wc -l)
        done
        #------------------------------
        echo "$(date "+%H:%M:%S") ($s3ActionName""Queue) - Start processing $queuedItem" >> "$logFile"
